    ...
    <>
      ...
      <aside className={`${isCollapsed ? 'w-24' : 'w-72'} h-screen border-r hidden lg:flex flex-col sticky top-0 left-0 z-40 transition-all duration-300 ease-in-out ${theme.bg}`}>
        <button
          ...
          className={`absolute -right-3.5 top-10 w-7 h-7 rounded-full flex items-center justify-center border shadow-md z-50 transition-all duration-200 hover:scale-110 ${theme.toggleBtn}`}
        >
        ...
        <div className={`p-8 flex items-center relative overflow-hidden transition-all duration-300 ${isCollapsed ? 'justify-center px-4' : 'gap-4'}`}>
          <div className="w-12 h-12 flex-shrink-0 relative z-10 rounded-xl overflow-hidden shadow-lg bg-gradient-to-br from-[#6c47ff] to-[#00d4aa]">
            <Link to="/"><img src="/logo.png" alt="Logo" className="w-full h-full object-cover" /></Link>
          </div>
          {!isCollapsed && (
        ...
        <nav className="flex-1 py-4 space-y-1.5 overflow-y-auto custom-scrollbar overflow-x-hidden">
        ...
        <div className={`p-4 border-t transition-all duration-300 ${isCollapsed ? 'px-2' : ''}`}>
          <div className={`rounded-2xl border flex flex-col transition-colors ${theme.userCard} ${isCollapsed ? 'p-2 items-center' : 'p-4'}`}>
            <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
              <div className="w-10 h-10 rounded-full bg-gray-700 flex items-center justify-center border-2 border-[#6c47ff] overflow-hidden">
                <User size={18} className="text-white" />
              </div>
              {!isCollapsed && <div className="min-w-0"><p className={`text-sm font-black truncate ${theme.text}`}>{user?.nom || 'Organisateur'}</p></div>}
            </div>
            <button onClick={handleLogout} className={`mt-4 flex items-center justify-center text-red-500 bg-red-500/10 hover:bg-red-500 hover:text-white transition-all duration-200 ${isCollapsed ? 'w-10 h-10 rounded-xl' : 'w-full gap-2 py-3 rounded-xl text-xs font-black uppercase tracking-widest'}`}>
      ...
      <nav className={`lg:hidden fixed bottom-0 left-0 w-full z-40 border-t pb-safe ${theme.bg}`}>
        <div className="flex items-center justify-around py-3 px-2">
          {/* ✅ ON FILTRE POUR GARDER LES 5 PLUS IMPORTANTS (Dashboard, Events, Tickets, Votes, Profil) */}
          {links.filter(l => l.name !== 'Stats').map((link) => {
            const isActive = location.pathname === link.path;
            return (
              <Link key={link.name} to={link.path} className={`flex flex-col items-center gap-1 transition-all ${isActive ? 'text-[#6c47ff] scale-110' : 'text-gray-400'}`}>
                {link.icon}
