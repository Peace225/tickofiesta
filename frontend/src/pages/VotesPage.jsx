...
function VoteCard({ vote, past = false }) {
  ...
  return (
    <Link 
      to={`/votes/${vote.id}`}
      className={`group relative flex flex-col justify-between rounded-[20px] p-5 overflow-hidden border border-[#2a304d] hover:border-blue-500 transition-all hover:shadow-[0_15px_40px_-10px_rgba(29,78,216,0.3)] min-h-[220px] ${past ? 'opacity-75 grayscale hover:grayscale-0' : ''}`}
    >
      ...
      <div className="mb-4 relative z-10">
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-[10px] font-black uppercase tracking-widest ${past ? 'bg-white/10 text-white/50' : 'bg-[#10b981] text-white shadow-lg shadow-green-500/20'}`}>
      ...
      <div className="relative z-10 mt-2">
        <p className="text-white/80 text-[10px] font-black uppercase tracking-[0.2em] mb-1 drop-shadow-md">
      ...
      <div className="flex items-end justify-between mt-auto relative z-10">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1 text-white/90 text-[12px] font-bold bg-black/40 backdrop-blur-sm px-2.5 py-1 rounded-lg border border-white/10">
            <User size={12} className="text-white/70" /> {vote.candidats?.length || 0}
          </div>
          {!past && (
            <div className="bg-[#facc15] text-black px-3 py-1 rounded text-[11px] font-black shadow-lg shadow-[#facc15]/20 uppercase tracking-widest hover:scale-105 transition-transform">
              Soutenir
            </div>
          )}
        </div>
        
        <span className="text-white/60 text-[11px] font-bold uppercase group-hover:text-white transition-colors flex items-center gap-1 bg-black/20 px-2 py-1 rounded-md backdrop-blur-sm border border-white/5">
      ...
      <section className="bg-[#0b1129] relative overflow-hidden text-white pt-24 pb-16 px-4 md:px-8 shadow-2xl">
        ...
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row items-start md:items-end justify-between gap-8">
          <div className="flex-1">
            <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-white/50 hover:text-white mb-6 text-[11px] font-black uppercase tracking-widest transition-colors">
              <ArrowLeft size={16} /> Retour
            </button>
            <div className="inline-flex items-center gap-2 bg-[#10b981] text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full mb-4 shadow-[0_0_15px_rgba(34,197,94,0.4)]">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 mt-10">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 space-y-10">
            ...
            <div className="bg-[#eef2ff] border border-blue-100 text-[#0052ff] p-4 rounded-xl flex items-center gap-3 shadow-sm">
              <AlertCircle size={20} className="text-[#0052ff] flex-shrink-0" />
              <span className="text-[13px] font-bold">Les résultats finaux seront révélés à la fin de chaque concours.</span>
            </div>

            {/* SECTION 1 : EN COURS */}
            <div>
              <div className="flex items-center gap-3 mb-6">
                <div className="w-1.5 h-6 bg-[#0052ff] rounded-full" />
              ...
              {actifs.length === 0 ? (
                <div className="text-center py-10 bg-white rounded-[20px] border border-gray-100">
                  <p className="font-bold text-slate-500 text-[14px]">Aucun concours en cours</p>
                </div>
              ) : (
          ...
          <div className="w-full lg:w-[320px] space-y-5">
            ...
            <div className="bg-gradient-to-b from-[#0052ff] to-[#0038b8] rounded-[20px] p-5 text-white shadow-lg shadow-blue-500/20 relative overflow-hidden">
              ...
              <div className="space-y-2.5 relative z-10">
                <button onClick={handleWhatsAppShare} className="w-full bg-[#25D366] hover:bg-[#20ba59] text-white py-2.5 rounded-xl font-bold text-[13px] transition-colors flex items-center justify-center gap-2 shadow-sm">
                  <MessageCircle size={18} /> Partager sur WhatsApp
                </button>
                <div className="flex gap-2.5">
                  <button onClick={handleCopy} className="flex-1 bg-white/10 hover:bg-white/20 py-2.5 rounded-xl text-[13px] font-bold transition-colors flex items-center justify-center gap-2">
                    {copied ? <CheckCircle size={14} /> : <Copy size={14} />} {copied ? 'Copié' : 'Copier'}
                  </button>
                  <button className="flex-1 bg-white/10 hover:bg-white/20 py-2.5 rounded-xl text-[13px] font-bold transition-colors flex items-center justify-center gap-2">
                    <Mail size={14} /> Email
                  </button>
                </div>
              </div>
              ...
              <div className="border-t border-white/10 pt-3 mt-4 relative z-10">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-[12px] font-medium text-white/90">Votes enregistrés</span>
                  <span className="text-[18px] font-black tracking-tight">{totalVotesCount > 0 ? totalVotesCount : '224'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-[20px] p-5 shadow-sm border border-gray-100">
              <p className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mb-4">Organisé par</p>
              <div className="flex items-center gap-4">
                <div className="w-11 h-11 rounded-xl bg-[#0052ff] text-white flex items-center justify-center font-bold text-lg shadow-inner">
            ...
            <div className="relative">
              {showShareMenu && (
                <div className="absolute bottom-full left-0 right-0 mb-3 bg-white border border-gray-100 shadow-xl rounded-2xl p-4 z-50 transition-all duration-300">
                  <p className="text-[10px] font-bold text-center text-slate-400 mb-3 uppercase tracking-widest">Partager sur</p>
                  <div className="flex justify-center gap-4">
                    ...
                    <button onClick={handleCopy} className="w-10 h-10 rounded-full bg-blue-50 text-[#0052ff] flex items-center justify-center hover:scale-110 transition-all">
              ...
              <button 
                ...
                className={`w-full py-3.5 rounded-xl text-[14px] font-bold transition-all flex items-center justify-center gap-2 relative z-10 ${showShareMenu ? 'bg-[#0052ff] text-white shadow-md shadow-blue-500/20' : 'bg-[#eef4ff] hover:bg-[#e0eaff] text-[#0052ff]'}`}
              >
