...
export default function LoginPage() {
  ...
  // Conversion téléphone -> email interne
  ...
  const handleSubmit = async (e) => {
    ...
    const email = idString.includes('@')? idString : phoneToEmail(idString)
    ...
    <div className={`min-h-screen flex transition-colors duration-500 ${theme.bg}`}>
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-[#05050a] flex-col justify-between p-16">
        <div className="absolute inset-0">
          <img src="/fond ecran evenement.jpg" alt="TickoFiesta Premium" className="w-full h-full object-cover opacity-30 scale-105" />
        ...
        <div className="relative z-10 text-white/20 text- font-bold uppercase tracking-[0.3em]">
        ...
        <div className="w-full max-w-md animate-in fade-in slide-in-from-bottom-6 duration-1000">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label className={`text- font-black uppercase tracking-[0.2em] opacity-50 ${theme.text}`}>
                Email ou Téléphone
              </label>
              <div className="relative group">
                ...
                <input
                  ...
                  placeholder="0102030405 ou email"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className={`text- font-black uppercase tracking-[0.2em] opacity-50 ${theme.text}`}>Mot de passe</label>
                <Link to="/forgot-password" className="text- font-bold text-[#6c47ff] hover:text-[#8b6bff] uppercase tracking-wider transition-colors">Oublié?</Link>
              </div>
              <div className="relative group">
                ...
                <input
                  ...
                  placeholder="••••••••"
                />
                <button type="button" onClick={() => setShowPwd(!showPwd)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-[#6c47ff] transition-colors p-1">
            ...
            <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-[#6c47ff] to-[#8b6bff] text-white py-5 rounded-2xl text-sm font-black uppercase tracking-[0.2em] shadow-2xl shadow-[#6c47ff]/30 transition-all active:scale-[0.98] flex items-center justify-center gap-4 mt-6 disabled:opacity-50 disabled:cursor-not-allowed hover:brightness-110">
              {loading? <Loader2 size={20} className="animate-spin" /> : <>Accéder au compte <ArrowRight size={20} /></>}
            </button>
          </form>
          ...
          <div className={`mt-12 pt-8 border-t ${dark? 'border-white/5' : 'border-slate-100'} text-center`}>
            <p className={`text-base font-medium ${theme.sub}`}>
              Pas encore membre? <Link to="/register" className="text-[#6c47ff] font-black hover:underline underline-offset-4 decoration-2">Créer un compte</Link>
            </p>
          </div>
        </div>
      </div>
