...
export default function Partenaires() {
  ...
  const theme = {
    card: dark ? 'bg-[#0f0e1a]/80 backdrop-blur-xl border-white/5 shadow-2xl' : 'bg-white border-gray-100 shadow-xl shadow-gray-200/40',
    text: dark ? 'text-white' : 'text-slate-900',
    sub: dark ? 'text-slate-400' : 'text-slate-500',
    input: dark ? 'bg-white/5 border-white/10 text-white placeholder:text-slate-500 focus:border-[#6c47ff]' : 'bg-white border-gray-200 text-slate-900 placeholder:text-slate-400 focus:border-[#6c47ff]',
    hover: dark ? 'hover:bg-white/5' : 'hover:bg-gray-50',
    modal: dark ? 'bg-[#0d0c16] border-white/5' : 'bg-white border-gray-100'
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in p-4 md:p-8 pb-32">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 bg-[#00d4aa]/10 border border-[#00d4aa]/20 rounded-full px-4 py-1.5 mb-3">
            ...
            <span className="text-[#00d4aa] text-xs font-black uppercase tracking-widest">
              Gestion Partenaires
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {[
          ...
          <div key={stat.label} className={`p-5 rounded-2xl border ${theme.card}`}>
            ...
            <p className={`text-xs font-bold uppercase tracking-widest mt-1 ${theme.sub}`}>{stat.label}</p>
          </div>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className={`py-20 text-center rounded-2xl border-2 border-dashed ${dark ? 'border-white/5' : 'border-gray-200'}`}>
          <Handshake size={48} className="mx-auto mb-4 opacity-20 text-slate-400" />
              ...
              <div className="p-5 space-y-3">
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    ...
                    className={`flex-1 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border transition-all disabled:opacity-50 ${
                      partner.actif
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-white/5 text-[#6c47ff] text-xs font-black uppercase tracking-widest border border-[#6c47ff]/20 hover:bg-[#6c47ff] hover:text-white transition-all"
                  >
                    <Edit size={14} />
                  </button>
                  <button
                    ...
                    disabled={processingId === partner.id}
                    className="px-4 py-2 rounded-xl bg-red-500/10 text-red-500 text-xs font-black uppercase tracking-widest border border-red-500/20 hover:bg-red-500 hover:text-white transition-all disabled:opacity-50"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className={`w-full max-w-2xl rounded-2xl border ${theme.modal} max-h-[90vh] overflow-y-auto shadow-2xl`}>
