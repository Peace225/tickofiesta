...
export default function RegisterPage() {
  ...
  // Convertit 0102030405 -> +2250102030405
  ...
  // Transforme le téléphone en email interne pour Supabase
  const phoneToEmail = (phone) => `${formatPhone(phone)}@tickofiesta.ci`
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.role === 'client') {
        phoneFormatted = formatPhone(form.telephone)
        emailToUse = phoneToEmail(form.telephone)
      }
      ...
      await finalizeRegistration(data.user, data.session, phoneFormatted)
    } catch (err) {
  ...
  const finalizeRegistration = async (authUser, session = null, phone = null) => {
    await supabase.from('profiles').upsert({
      id: authUser.id,
      nom: form.nom,
      email: form.role === 'organisateur'? form.email : phoneToEmail(form.telephone),
      telephone: phone,
      role: form.role,
      isActive: true
    }, { onConflict: 'id' })
