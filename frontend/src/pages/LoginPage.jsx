...
export default function LoginPage() {
  ...
  const [supaUser, setSupaUser] = useState(null); // <-- AJOUT PARFAIT
  const from = location.state?.from?.pathname || null;

  // 1. Écoute Supabase (Google/Facebook) et sync
  useEffect(() => {
  ...
  const effectiveUser = user || supaUser; // <-- on utilise Redux OU Supabase

  // 2. Redirection après connexion
  useEffect(() => {
    if (effectiveUser) {
      const rawRole = effectiveUser?.role || effectiveUser?.user_metadata?.role || 'client';
      ...
      if (['admin', 'super_admin', 'superadmin'].includes(userRole)) {
        ...
        navigate(from && !['/login', '/register'].includes(from) && !from.startsWith('/admin') && !from.startsWith('/dashboard') ? from : '/', { replace: true });
      }
    }
      ...
      <div className="hidden lg:flex lg:w-[45%] relative items-center justify-center overflow-hidden bg-black">
        ...
        <div className="relative z-10 text-center px-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
          ...
          <p className="mt-6 text-xl leading-relaxed text-white/70 max-w-sm mx-auto font-medium">
            Vos billets, vos votes, en un seul geste. L'expérience premium commence ici.
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-[440px] animate-in fade-in zoom-in-95 duration-700">
          ...
          <div className={`backdrop-blur-3xl rounded-[2rem] border p-10 lg:p-12 ${apple.card}`}>
            <div className="text-center mb-10">
              <h1 className={`text-3xl font-bold tracking-tight ${apple.text}`}>Connexion</h1>
              <p className={`text-sm mt-2 font-medium ${apple.sub}`}>Accédez à votre compte personnel</p>
            </div>

            <div className={`flex items-center p-1.5 rounded-2xl mb-8 transition-colors ${apple.socialBox}`}>
            ...
            <div className="relative flex items-center my-8">
              ...
              <span className={`px-4 text-xs font-semibold uppercase tracking-widest ${apple.sub}`}>Ou via email</span>
              <div className="flex-1 h-px bg-black/5 dark:bg-white/10" />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="relative group">
                ...
                <button 
                  ...
                  className={`absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-xl hover:bg-black/5 dark:hover:bg-white/10 transition-colors ${apple.sub}`}
                >
                  {showPwd ? <EyeOff size={18} /> : <Eye size={18} />}
                </button>
              </div>
              <div className="flex justify-end pt-1 pb-2">
                <Link to="/forgot-password" className="text-sm font-semibold text-[#0071e3] hover:text-[#005bb5] transition-colors">Mot de passe oublié ?</Link>
              </div>
              <button 
                type="submit" 
                disabled={loading} 
              ...
              >
                {loading ? <Loader2 size={20} className="animate-spin" /> : <>Se connecter <ArrowRight size={18} className="opacity-80" /></>}
              </button>
            </form>
          </div>
          
          <p className={`text-center text-[15px] font-medium mt-10 ${apple.sub}`}>
            Nouveau sur TickoFiesta ?{' '}
            <Link to="/register" className="text-[#0071e3] font-semibold hover:text-[#005bb5] transition-colors">Créer un compte</Link>
          </p>
        </div>
      </div>
    </div>
